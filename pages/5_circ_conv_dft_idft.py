...
import copy
st.markdown('## Circular Convolution using DFT and IDFT')
st.markdown('#### Determines the Circular convolution b/w x(n) & h(n) using DFT and IDFT')
...
if x1 and h1:
    ...
    if len(x2)>len(h2):
        N=len(x)
        for i in range(len(h2),len(x2)):
            h2.append(0)
    elif len(x)<len(h):
        N=len(h)
        for i in range(len(x2),len(h2)):
            x2.append(0)
    else:
        N=len(x)
    X2=np.fft.fft(x2)
    ...
    st.write('Circular convolution using DFT and IDFT y=',y)
    fig=plt.figure()
    plt.subplot(3,1,1)
